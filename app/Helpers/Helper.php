...
public static function getusername(int $userid)
    ...
    public static function shipment_type(string $id) {
        $query = DB::table('shipment_type')->where('id', $id)->first();
        if ($query !== null) {
            return $query->name;
        } else {
            return '-';
    ...
    public static function services_required(string $id) {
        $query = DB::table('services_required')->where('id', $id)->first();
        if ($query !== null) {
            return $query->name;
        } else {
            return '-';
    ...
    public static function time_zonenamenew(string $string)
{
    // Convert comma-separated string to array
    $ids = array_filter(array_map('trim', explode(',', $string)));

    // Fetch all matching time zones
    $zones = DB::table('surveyor_time_zone')
        ->whereIn('id', $ids)
        ->pluck('time_zone')
        ->toArray();

    // Return comma-separated string of time zone names
    return implode(', ', $zones);
}

    public static function vehicle(string $id) {
        $query = DB::table('vehicales')->where('id', $id)->first();
        if ($query !== null) {
            return $query;
        } else {
            return '-';
    ...
    public static function materialname(string $id) {
        $query = DB::table('materials')->where('id', $id)->first();
        if ($query !== null) {
            return $query;
        } else {
            return '-';
    ...
    public static function branchname(string $id) {
        $query = DB::table('branch')->where('id', $id)->first();
        if ($query !== null) {
            return $query->branch;
        } else {
            return '-';
