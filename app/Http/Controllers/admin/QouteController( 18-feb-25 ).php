    ...
    public function send_quotation_mail(Request $request){
       ...
       /*  try{ */
        ...
        $htmlAdmin .= '<!DOCTYPE html>
                ...
                $pdf = LaravelMpdf::loadHTML($mailFormatType);
                // ✅ Use the mPDF instance to set paper size and footer
                $pdf = $pdf->getMpdf();
                $pdf->SetDisplayMode('fullpage'); // Optional: Improves display mode
                $pdf->SetMargins(10, 10, 10, 30); // Adjust bottom margin
                $pdf->SetAutoPageBreak(true, 30); // Add auto page break
                $pdf->SetHTMLFooter('
                    <div style="width: 100%; text-align: center; font-size: 10pt;">
                        <b>For : QUICKSERVE RELOCATIONS LLC -2024-25</b><br>
                        <img src="'.public_path('admin/assets/img/erp-sign.png').'" style="width: 150px; display: block; margin: 10px auto;">
                        <p style="text-align: right; margin-right: 50px;">Authorised Signatory</p>
                    </div>
                ');

                // ✅ Set paper size using `mpdf` directly
                // $pdf->SetPageSize('A4');
                // $pdf->SetOrientation('P'); // 'P' for Portrait, 'L' for Landscape
