    ...
    <div class="" style="width:100%">
        ...
        <div class="quote" style="width: 100%;display: inline-block; text-align:right">
            ...
            @if($followup_data->accepted_quotation == 0)
                <a href="{{ route('request.accept', ['enquiry_id' => $followup_data->id, 'format_type' => 2]) }}" style="text-decoration: none;{{ $acceptQuoteStyle }}">
                    <button type="button"
                ...
                <tr>
                    <th rowspan="6"
                        ...
                        @if (isset($clientName) && !empty($clientName))
                            <p style="margin-top: 0;margin-bottom: 5px;">To:&nbsp;{{ $clientName }}
                        ...
                        @if (isset($followup_data->customer_email) && !empty($followup_data->customer_email))
                            <p style="margin-top: 0;margin-bottom: 5px;">EMAIL
                                ID:&nbsp;{{ $followup_data->customer_email ?? "" }}</p>
                        @endif
                        ...
                        @if (isset($followup_data->f_name) && !empty($followup_data->f_name) ||
                            ...
                            {{-- <br /><p style="margin-top: 0;margin-bottom: 5px;">Customer Detiails : </p> --}}
                            @if (isset($followup_data->f_name) && !empty($followup_data->f_name) ||
                                isset($followup_data->l_name) && !empty($followup_data->l_name))
                                    {{-- <p style="margin-top: 0;margin-bottom: 5px;">
        ...
        <div class="test">
            <table style="width:100%">
                <tr style="background: #0056b3;color: #FFFFFF !important;">
                    ...
                    <th style="padding: 5px 0px 5px 0px;color: #FFFFFF !important;">Amount(AED)</th>
                </tr>
                @if (isset($costing_attribute) && !empty($costing_attribute))
                    ...
                    @endphp
                    @foreach ($costing_attribute as $costing)
                    @php
                        $marginOfTotal = ($costing->total * $followup_data->margin_percent) / 100;
                        $total = $costing->total + $marginOfTotal;
                    @endphp
                        <tr>
                            <td style="padding: 10px 0px 10px 10px;">{{ $i }}</td>
                            <td style="padding: 10px 0px 10px 10px;">{{ $costing->description }}</td>
                    ...
                    @endforeach
                    @php
                        $totalvat_amount = ($totalAmount * 5) / 100;
                        $total = $followup_data->selling_amount * 1.05;
                    @endphp

                @endif
                {{-- <tr>
                    <td colspan="2" style="padding: 10px 0px 10px 10px;">Total</td>
                    <td style="padding: 10px; text-align: right;">

                        {{ number_format($totalAmount, 2, '.', ',') }}</td>
                </tr>
                <tr>
                    <td colspan="2" style="padding: 10px 0px 10px 10px;">Selling Sum</td>
                    <td style="padding: 10px; text-align: right;">

                        {{ number_format($followup_data->selling_amount, 2, '.', ',') }}</td>

                </tr> --}}
                @if (isset($quotation_data->vat_charge) && $quotation_data->vat_charge == 1)
                    <tr>
                        <td colspan="2" style="padding: 10px 0px 10px 10px;"> VAT (5 %)</td>
                        <td style="padding: 10px; text-align: right;">
                            {{ number_format($totalvat_amount, 2, '.', ',') }}
                        </td>

                    </tr>
                @endif
                <tr>
                    <td colspan="2" style="padding: 10px 0px 10px 10px;"> Estimated Total</td>
                    {{-- @if (isset($quotation_data->vat_charge) && $quotation_data->vat_charge == 1)
                        <td style="padding: 10px; text-align: right;">{{ number_format($total, 2, '.', ',') }}</td>
                    @else
                        <td style="padding: 10px; text-align: right;">{{ number_format($followup_data->grand_total, 2, '.', ',') }}</td>
                    @endif --}}

                   
                    <td style="padding: 10px; text-align: right;">{{ $followup_data->grand_total_with_vat ?? "0.00" }}</td>
                </tr>
       ...
       {{--  <div class="incl" style="font-size: 20px;">
        ...
        {{-- <div style="width: 100%;display: inline-block;">
            ...
            <img src="{{ asset('public/admin/assets/img/erp-sign.png') }}" style="width: 30%;float: right;margin-right:60px;">
