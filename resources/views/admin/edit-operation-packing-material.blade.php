                                            ...
                                            <tbody>
                                                ...
                                                @forelse ($material_data as $data)
                                                    <tr>
                                                        ...
                                                        <td>{{ $data->name }}</td>
                                                        <td>
                                                            {{ Helper::getSumOfMaterialStocks($data->id)}} {{$data->materal_def }}
                                                        </td>
                                                        ...
                                                        @php
                                                            $totalCost = Helper::getMaterialTotalCosts($followup->id,$data->id)
                                                        @endphp

                                                        <td class="amount-input">
                                                            <input type="text" class="form-control" name="total_cost[]" id="total_cost" value="{{ $totalCost }}" readonly></td>

                                                        <td class="text-center">
                                                            @if($totalCost !="" && !empty($totalCost))
                                                            <button type="button" 
                                                                    class="btn btn-primary"
                                                                    data-bs-toggle="modal" data-bs-target="#packingMaterialReturnModal{{ $data->id }}">Return</button>
                                                            @endif
                                                        </td>
                                                    </tr>
                                                @empty
            ...
            <div class="modal-content">
                <form id="warehouse_allocateForm{{ $data->id }}" action="{{ route('warehouse.allocate') }}" method="POST">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td draggable="false">
                                        ...
                                        <input type="hidden" name="godown_id_new[]" value="{{ $warehouse->godown_id }}">
                                    </td>
                                    <td draggable="false">{{ $quotation_data->purchase_order_no ?? "" }}</td>
                                    <td draggable="false">{{ $warehouse->stock }} {{$data->materal_def}}</td>
                                    <input type="hidden" 
                                               name="allocat_stock_new[]" 
                                               id="quantity_stock_new_{{ $data->id }}_{{ $warehouse->godown_id }}" 
                                    ...
                                    <td draggable="false">
                                        <input type="text" 
                                               class="form-control" 
                                               id="allocate_{{ $data->id }}_{{ $warehouse->godown_id }}" 
                                               name="allocate[]" 
                                               value="{{ Helper::getEnteredAllocat($followup->id,$data->id,$warehouse->godown_id) }}"
                                        />
                                        <p style="color: red;" id="allocate-input-error_{{ $data->id }}_{{ $warehouse->godown_id }}"></p>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" id="warehouse_submit_button{{ $data->id }}" onclick="warehouse_allocateSubmit('{{ $data->id }}');">Save changes</button>
...
@endforeach
    ...
    @forelse ($material_data as $data)
    <div class="modal fade warehouse-popup-table custom-modal" id="packingMaterialReturnModal{{ $data->id }}" tabindex="-1" role="dialog" aria-labelledby="packingMaterialReturnModal{{ $data->id }}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form id="warehouseReturnForm{{ $data->id }}" action="{{ route('warehouse.return') }}" method="POST">
                    @csrf
                    <input type="hidden" name="material_id" value="{{ $data->id }}">
                    <input type="hidden" name="enquiry_id" value="{{ $followup->id }}">
                    <div class="modal-header">
                    ...
                    @php
                        $quotation_packing_material = Helper::getQuotationPackingMaterial($followup->id,$data->id);
                    @endphp
                    ...
                    <div class="modal-body text-center" style="max-height: 400px; overflow-y: auto;">
                        <table class="table table-hover mb-0 warehouse-table">
                            ...
                            <tbody>
                                @if(is_iterable($quotation_packing_material))
                                @forelse($quotation_packing_material as $index => $warehouse)
                                @php
                                    $godownName   = Helper::getGodownName($warehouse->godown_id);
                                    $exitsAllocat = Helper::getEnteredAllocat($followup->id,$data->id,$warehouse->godown_id).' Boxes';
                                    $totalCost    = Helper::getPackingMaterialPrice($data->id,$warehouse->godown_id);
                                @endphp
                                <tr>
                                    <td draggable="false">{{ $loop->iteration }}</td>
                                    <td draggable="false">
                                    ...
                                    <td draggable="false">
                                        ...
                                        <input type="hidden" 
                                               name="allocat_stock[]" 
                                               id="allocat_stock_{{ $data->id }}_{{ $warehouse->godown_id }}" 
                                               value="{{ Helper::getEnteredAllocat($followup->id, $data->id, $warehouse->godown_id) }}"
                                        />
                                    </td>
                                    <td draggable="false">{{ $totalCost }}</td>
                                    <td draggable="false">
                                        <input type="text" 
                                               class="form-control" 
                                               id="return_allocate_{{ $data->id }}_{{ $warehouse->godown_id }}" 
                                               name="return_allocate[]" 
                                               value=""
                                        />
                                        <p style="color: red;" id="return-input-error_{{ $data->id }}_{{ $warehouse->godown_id }}"></p>
                                    </td>
                                </tr>
                                @empty
                                    <tr>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" 
                                class="btn btn-primary" 
                                id="warehouse_return_submit_button{{ $data->id }}" 
                                onclick="warehouseReturnSubmit('{{ $data->id }}');">Save changes</button>
                        <button class="btn btn-primary mb-1" type="button" disabled id="warehouse_return_spinner_button{{ $data->id }}"
                            style="display: none;">
                            <span class="spinner-border spinner-border-sm" role="status"
                                aria-hidden="true"></span>
                            Loading...
...
@section('footer_js')
    ...
    <script>
        ...
        function warehouse_allocateSubmit(formId) {
            var isValid = true;
            var atLeastOneValidNew = false; // Flag to check if at least one valid allocate is entered

            jQuery(`#warehouse_allocateForm${formId} tbody tr`).each(function () {
                var godownId = jQuery(this).find("input[name='godown_id_new[]']").val();
                var availableQty = parseFloat(jQuery(`#quantity_stock_new_${formId}_${godownId}`).val()) || 0;
                var allocatStock = parseFloat(jQuery(`#allocate_${formId}_${godownId}`).val()) || 0;

                var errorElement = jQuery(`#allocate-input-error_${formId}_${godownId}`);
                errorElement.html("").hide(); // Reset previous errors

                console.log(`Godown ID: ${godownId}, Allocated: ${allocatStock}, Available: ${availableQty}`);

                // Check if at least one valid allocation is entered
                if (!isNaN(allocatStock) && allocatStock > 0) {
                    atLeastOneValidNew = true;
                }
                
                // Validation: Allocation should not be greater than available quantity
                if (!isNaN(allocatStock) && allocatStock > availableQty) {
                errorElement.html("The allocate value cannot be <br/>greater than the available quantity.").show();
                    isValid = false;
                }
            });

            // If no valid allocation is entered, prevent submission
            if (!atLeastOneValidNew) {
                alert("Please enter a valid allocate value in at least one row.");
                return false;
            }
            ...
            // Show spinner button and hide submit button
            jQuery(`#warehouse_submit_button${formId}`).hide();
            jQuery(`#warehouse_spinner_button${formId}`).show();

            // Submit the form
            jQuery(`#warehouse_allocateForm${formId}`).submit();
        }

       
        
        function warehouseReturnSubmit(formId) {
            var isValid = true;
            var atLeastOneValidReturn = false; // Flag to track if at least one valid returnStock is entered

            jQuery(`#warehouseReturnForm${formId} tbody tr`).each(function () {
                var godownId      = jQuery(this).find("input[name='godown_id[]']").val();
                var allocatStock  = parseFloat(jQuery(`#allocat_stock_${formId}_${godownId}`).val()) || 0;
                var returnStock   = parseFloat(jQuery(`#return_allocate_${formId}_${godownId}`).val()) || NaN;

                var errorElement  = jQuery(`#return-input-error_${formId}_${godownId}`);

                // Reset previous errors
                errorElement.html("").hide();

                console.log(`Godown ID: ${godownId}, Allocated: ${allocatStock}, Return: ${returnStock}`);

                // Validation checks
                if (!isNaN(returnStock)) {
                    atLeastOneValidReturn = true; // At least one valid returnStock is entered

                    if (returnStock > allocatStock) {
                        errorElement.html("The return quantity cannot be <br/>greater than the allocated quantity.")
            ...
            // Prevent form submission if no valid returnStock is entered or validation fails
            if (!atLeastOneValidReturn) {
                console.log("No valid return quantity entered, form will not be submitted.");
                alert("Please enter a valid return quantity in at least one row.");
                return false;
            }
            ...
            // Show loading button and submit form
            jQuery(`#warehouse_return_submit_button${formId}`).hide();
            jQuery(`#warehouse_return_spinner_button${formId}`).show();
            jQuery(`#warehouseReturnForm${formId}`).submit();
        }
    ...
    <script>
        ...
        // function returnPackingMaterial() {
        //     $('#packingMaterialReturnModal').modal('show');
        // }
